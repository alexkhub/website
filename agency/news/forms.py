...
class Add_Real_estateForm(forms.Form):
    ...
    city = forms.ModelChoiceField(queryset=City.objects.all(), label='Город', empty_label='Город не выбран', widget=forms.TextInput(attrs={'class': 'form-add-real-estate-choice'}))
    ...
    category_name = forms.ModelChoiceField(queryset=Сategories_real_estate.objects.all(), label='Категория', empty_label='Категория не выбрана', widget=forms.TextInput(attrs={'class': 'form-add-real-estate-choice'}))
    services = forms.ModelChoiceField(queryset=Services.objects.all(), label='Продать / Сдать в аренду', empty_label='Услуга не выбрана', widget=forms.TextInput(attrs={'class': 'form-add-real-estate-choice'}))
